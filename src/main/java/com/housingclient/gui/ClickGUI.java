...
public class ClickGUI extends GuiScreen {
    ...
    // Icon Caching
    private static final Map<String, ResourceLocation> ICON_CACHE = new HashMap<String, ResourceLocation>();
    ...
    public void initGui() {
        ...
        // Always create a fresh search field with the current fontRendererObj to avoid
        // stale font renderer references that cause intermittent search failures.
        // Preserve the previous search text across re-opens (e.g. window resize).
        String oldText = (searchField != null) ? searchField.getText() : null;
        searchField = new GuiTextField(0, fontRendererObj, 0, 0, 140, 22);
        searchField.setFocused(true);
        searchField.setCanLoseFocus(false);
        searchField.setText(oldText != null ? oldText : "");
        if (oldText == null) {
            // First open - reset all GUI state
            scrollOffset = 0;
            targetScroll = 0;
            settingsModule = null;
            settingsScroll = 0;
    ...
    private void drawModuleCard(int x, int y, Module module, int mouseX, int mouseY) {
        ...
        String path = "textures/icons/" + iconName + ".png";
        ResourceLocation icon = ICON_CACHE.get(path);
        if (icon == null) {
            icon = new ResourceLocation("housingclient", path);
        ...
        for (Setting<?> setting : settingsModule.getSettings()) {
            ...
            if (currentY + 30 >= contentY && currentY <= y + GUI_HEIGHT) {
                ...
                if (setting instanceof BooleanSetting) {
                    ...
                    int boxH = 16;
                    int boxX = itemX + itemWidth - boxW;
                    int boxY = (int) currentY + 4;

                    boolean isFocused = (focusedStringSetting == strSet);
                    int borderColor = isFocused ? ACCENT_GREEN : 0xFF555555;

                    RenderUtils.drawRoundedRect(boxX, boxY, boxW, boxH, 3, 0xFF1A1A1A);
                    RenderUtils.drawRoundedRectOutline(boxX, boxY, boxW, boxH, 3, borderColor, 1f);

                    String displayStr = strSet.getValue();
                    if (isFocused && (System.currentTimeMillis() / 500) % 2 == 0) {
                        displayStr += "_";
                    }

                    // Simple overflow handling, just clip via scissor or by character cutoff
                    if (fontRendererObj.getStringWidth(displayStr) > boxW - 8) {
                        displayStr = fontRendererObj.trimStringToWidth(displayStr, boxW - 12) + "...";
                    }

                    fontRendererObj.drawString(displayStr, boxX + 4, boxY + 4, isFocused ? 0xFFFFFFFF : TEXT_GRAY);
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (settingsModule != null) {
            ...
            float currentY = contentY + 20 - settingsScroll;
            focusedStringSetting = null; // Clear focus unless a new valid click occurs
                ...
                if (currentY + 20 >= contentY && currentY <= guiY + GUI_HEIGHT && mouseY >= currentY
                    ...
                    if (setting instanceof BooleanSetting) {
                        ...
                        if (mouseX >= boxX && mouseX < boxX + boxW && mouseY >= boxY && mouseY < boxY + boxH) {
                            focusedStringSetting = (StringSetting) setting;
                            return;
                        }
                    }
                }
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        // First handle typing in focused StringSetting
        if (focusedStringSetting != null) {
            if (keyCode == Keyboard.KEY_ESCAPE) {
                focusedStringSetting = null;
                return;
            } else if (keyCode == Keyboard.KEY_BACK || keyCode == Keyboard.KEY_DELETE) {
                String current = focusedStringSetting.getValue();
                if (!current.isEmpty()) {
                    focusedStringSetting.setValue(current.substring(0, current.length() - 1));
                }
                return;
            } else if (keyCode == Keyboard.KEY_RETURN || keyCode == Keyboard.KEY_NUMPADENTER) {
                focusedStringSetting = null;
                return;
            } else if (GuiScreen.isCtrlKeyDown() && keyCode == 47) { // 47 = V key
                String clipboard = GuiScreen.getClipboardString();
                if (clipboard != null && !clipboard.isEmpty()) {
                    String current = focusedStringSetting.getValue();
                    String pasteText = clipboard.split("\n")[0].split("\r")[0];
                    String newText = current + pasteText;
                    if (newText.length() > focusedStringSetting.getMaxLength()) {
                        newText = newText.substring(0, focusedStringSetting.getMaxLength());
                    }
                    focusedStringSetting.setValue(newText);
                }
                return;
            } else if (typedChar >= 32 && typedChar != 127) { // Allow all non-control unicode chars
                String current = focusedStringSetting.getValue();
                if (current.length() < focusedStringSetting.getMaxLength()) {
                    focusedStringSetting.setValue(current + typedChar);
                }
                return;
            }
            // Consume all other keys while text input is focused
            return;
        }
