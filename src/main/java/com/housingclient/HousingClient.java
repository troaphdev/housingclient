...
public class HousingClient {
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        // --- BungeeCord Packet Flush ---
        // If we officially transitioned into Play mode, dump any trapped packets!
        if (mc.getNetHandler() instanceof net.minecraft.client.network.NetHandlerPlayClient) {
            while (!com.housingclient.utils.BungeePacketQueue.delayPacketQueue.isEmpty()) {
                net.minecraft.network.Packet<?> p = com.housingclient.utils.BungeePacketQueue.delayPacketQueue.poll();
                try {
                    @SuppressWarnings("unchecked")
                    net.minecraft.network.Packet<net.minecraft.network.INetHandler> rawPacket = (net.minecraft.network.Packet<net.minecraft.network.INetHandler>) p;
                    rawPacket.processPacket(mc.getNetHandler());
                } catch (Exception e) {
                    LOGGER.error("[HousingClient] Error processing BungeeCord queued packet: ", e);
                }
            }
        }
