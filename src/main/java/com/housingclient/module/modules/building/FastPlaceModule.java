    ...
    public void onTick() {
        ...
        if (mc.currentScreen != null) {
            ...
            if (mc.currentScreen instanceof net.minecraft.client.gui.inventory.GuiEditSign) {
                com.housingclient.module.Module signFill = com.housingclient.HousingClient.getInstance()
        ...
        // Only when holding right click (check keybind instead of strict mouse bounds
        // because closing GUIs can sometimes ungrab the mouse briefly)
        ...
        // Ignore block swap cooldown if we're aggressively filling signs
        boolean bypassCooldown = false;
        if (currentItem instanceof net.minecraft.item.ItemSign) {
            com.housingclient.module.Module signFill = com.housingclient.HousingClient.getInstance()
                    .getModuleManager().getModule(com.housingclient.module.modules.exploit.SignFillModule.class);
            if (signFill != null && signFill.isEnabled()) {
                bypassCooldown = true;
        ...
        if (swapCooldown > 0 && !bypassCooldown) {
            return;
        }
